...
namespace TripPlanner.Services.BillService
{
    public class BillService : IBillService
    {
        ...
        public BillService(ITransferRepository TransferRepository, IBillRepository BillRepository, 
            IBillContributorRepository billContributorRepository, ITourRepository tourRepository, 
        ...
        private async Task<string> GetUserFullNameOrNickname(int tourId, int userId)
        {
            string name = "";
        ...
        private async Task<RepositoryResponse<BillPresentationDTO>> GetBillPresentation(int userId, int billId, int tourId)
        {
        ...
        private async Task<RepositoryResponse<TransferPresentationDTO>> GetTransferPresentation(int userId, int transferId, int tourId)
        {
        ...
        private async Task<RepositoryResponse<Balance>> GetBalance(int tourId)
        {
            ...
            var tourDB = await _TourService.GetTourAsync(u => u.Id == tourId, "Participants.User"); //TODO sprawdzic czy dziala, chyba bedzie ok
        ...
        public async Task<RepositoryResponse<bool>> UpdateTransfer(CreateTransferDTO Bill)
        {
            var response = await _TransferRepository.Update(Tour);
            if (response.Success == false)
            {
                return response;
            }
